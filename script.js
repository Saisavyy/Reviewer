...
checkBtn.addEventListener('click', () => {
  ...
  for (const [key, value] of Object.entries(answers)) {
    ...
    // Show correction note when answer is wrong or missing
    // Remove previous correction if present
    const existingCorr = exp.querySelector('.correction');
    if (existingCorr) existingCorr.remove();
    let correctText = '';
    if (qElem) {
      const correctLabel = qElem.querySelectorAll('label');
      correctLabel.forEach(lbl => {
        const inp = lbl.querySelector('input');
        if (inp && inp.value === value) correctText = lbl.textContent.trim();
      });
    }
    if (!user || (user && user !== value)) {
      const corr = document.createElement('div');
      corr.className = 'correction';
      corr.textContent = `Correct answer: ${correctText}`;
      exp.appendChild(corr);
    }
  }
  ...
  // Reset quiz: clear selections, highlights, explanations, corrections, and result
  const resetBtn = document.getElementById('resetBtn');
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      const form = document.getElementById('quizForm');
      if (form) form.reset();
      // Clear per-question UI
      for (const key of Object.keys(answers)) {
        const qElem = document.getElementById(key);
        if (qElem) {
          const labels = qElem.querySelectorAll('label');
          labels.forEach(lbl => lbl.classList.remove('correct', 'incorrect', 'chosen'));
        }
        const exp = document.getElementById(`exp-${key}`);
        if (exp) {
          exp.style.display = 'none';
          exp.classList.remove('correct', 'incorrect');
          const corr = exp.querySelector('.correction');
          if (corr) corr.remove();
        }
      }
